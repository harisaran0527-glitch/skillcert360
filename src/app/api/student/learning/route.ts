  ...
  try {
    ...
    await studentTransaction(profile.id, async tx => {
      ...
      if (action === "learn") {
        ...
        if (!course || !/^https?:\/\//i.test(course.officialUrl) || !URL.canParse(course.officialUrl) || course.officialUrl.includes("official-provider.org")) {
          throw new WorkflowError("Select an active official course", 400);
        }

        if (!enrollment) {
          enrollment = await tx.studentSkill.create({ data: { studentId: profile.id, skillId } });
          await transition(tx, profile.id, skillId, ["LEARNING"]);
        }

        if (enrollment.selectedCourseId !== course.id) {
          if (enrollment.completedAt) {
            throw new WorkflowError("The selected course cannot change after learning is completed.");
          }
          await tx.studentSkill.update({ where: { id: enrollment.id }, data: { selectedCourseId: course.id } });
        }

        await tx.courseProgress.upsert({
          where: { studentSkillId_courseId: { studentSkillId: enrollment.id, courseId: course.id } },
          create: { studentSkillId: enrollment.id, courseId: course.id },
          update: {}
        });
        ...
        await tx.activityLog.create({
          data: {
            ...
            action: "OFFICIAL_COURSE_OPENED",
            metadata: { skillId, courseId: course.id, providerId: course.providerId }
          }
        });
      } else {
        if (!enrollment) throw new WorkflowError("Start an official course first.");
        if (enrollment.completedAt) return;
        if (!enrollment.selectedCourseId || !await tx.courseProgress.count({ where: { studentSkillId: enrollment.id, courseId: enrollment.selectedCourseId } })) {
          throw new WorkflowError("Choose and open an official course before marking learning complete.");
        }

        await tx.courseProgress.updateMany({
          where: { studentSkillId: enrollment.id, courseId: enrollment.selectedCourseId },
    ...
    return request.headers.get("content-type")?.includes("application/json")
      ? Response.json({ ok: true, redirect: action === "complete" ? destination.pathname : undefined })
