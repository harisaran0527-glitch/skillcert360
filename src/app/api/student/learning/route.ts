...
export async function POST(request: Request) {
 ...
 try {
  ...
  await studentTransaction(profile.id, async tx => {
   ...
   if (action === "learn") {
    const course = await tx.course.findFirst({ where: { id: courseId ?? "", skillId, active: true, provider: { active: true } } });
    if (!course || !/^https?:\/\//i.test(course.officialUrl)) throw new WorkflowError("Select an active official course", 400);
    ...
    await tx.activityLog.create({ data: { studentId: profile.id, action: "OFFICIAL_COURSE_OPENED", metadata: { skillId, courseId: course.id } } });
   } else {
    if (!enrollment) throw new WorkflowError("Start an official course first.");
    if (enrollment.completedAt) return;
    if (!await tx.courseProgress.count({ where: { studentSkillId: enrollment.id } })) throw new WorkflowError("Open an official course before marking learning complete.");
    await tx.courseProgress.updateMany({ where: { studentSkillId: enrollment.id }, data: { completedAt: new Date() } });
  ...
  return request.headers.get("content-type")?.includes("application/json") ? Response.json({ ok: true }) : NextResponse.redirect(new URL("/student/skills", request.url), 303);
