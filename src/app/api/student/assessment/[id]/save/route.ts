...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json(await assessmentAction(session.userId, id, parsed.data.clientId, "save", parsed.data.responses, parsed.data.types), { headers: { "Cache-Control": "no-store" } });
