...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!parsed.data.types?.length) return Response.json({ error: "Violation type required" }, { status: 400 });
    return Response.json(await assessmentAction(session.userId, id, parsed.data.clientId, "violation", parsed.data.responses, parsed.data.types), { headers: { "Cache-Control": "no-store" } });
